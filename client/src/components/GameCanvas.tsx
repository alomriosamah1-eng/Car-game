...
type Flow = "menu" | "selection" | "customize" | "environment" | "preDrive";
...
export default function GameCanvas() {
  ...
  const [environment, setEnvironment] = useState("coastal");
  ...
  const play = () => { setPanel("home"); setDriveStarted(true); gameRef.current?.startDrive(); unlock(); };
  const startFromPreDrive = () => { setDriveStarted(true); setFlow("menu"); gameRef.current?.startDrive(); unlock(); };
  const togglePause = () => { const next = !paused; setPaused(next); gameRef.current?.setPaused(next); };
  const exitToMenu = () => { setPaused(false); gameRef.current?.setPaused(false); setDriveStarted(false); setFlow("menu"); };
  ...
  return <div className={`game-shell ${rtl ? "rtl" : "ltr"}`} dir={rtl ? "rtl" : "ltr"} onPointerDown={unlock}>
    ...
    <header className="top-bar">
      ...
      <div className="top-actions"><GlassButton ariaLabel={paused ? "Resume" : "Pause"} onClick={togglePause} active={paused}>{paused ? <Play size={17} /> : <Pause size={17} />}</GlassButton><GlassButton ariaLabel={t.settings} onClick={() => setPanel(panel === "settings" ? "home" : "settings")} active={panel === "settings"}><Settings2 size={17} /></GlassButton><GlassButton ariaLabel={t.language} onClick={() => setLanguage(language === "ar" ? "en" : "ar")}><Languages size={17} /></GlassButton></div>
    </header>
    ...
    {paused && <div className="pause-overlay"><div className="pause-card"><div className="eyebrow">DRIVE / PAUSED</div><h2>{language === "ar" ? "توقفت الرحلة مؤقتًا" : "Drive paused"}</h2><p>{language === "ar" ? "خذي لحظة، ثم عودي للطريق عندما تكونين جاهزة." : "Take a moment, then return when you are ready."}</p><button className="primary-cta" onClick={togglePause}><Play size={17} /> {language === "ar" ? "متابعة" : "Resume"}</button><button className="secondary-cta" onClick={() => { gameRef.current?.reset(); togglePause(); }}>{language === "ar" ? "إعادة الجولة" : "Restart"}</button><button className="secondary-cta" onClick={exitToMenu}>{language === "ar" ? "العودة للقائمة" : "Exit to menu"}</button></div></div>}
    </>}
    {!driveStarted && <section className="onboarding" aria-label="Main menu">
      <div className="onboarding-brand"><div className="brand-mark">V</div><div className="brand-name">VEL OURA</div><div className="brand-sub">DRIVE / 01</div></div>
      {flow === "menu" && <div className="onboarding-panel"><div className="eyebrow">BLUE HOUR / OFFLINE DRIVE</div><h1>Veloura <em>Drive</em></h1><p>{language === "ar" ? "طريق ساحلي هادئ، مصمم ليمنحك حضورًا كاملًا خلف المقود." : "A quiet coastal route, designed for a full presence behind the wheel."}</p><button className="primary-cta onboarding-primary" onClick={() => setFlow("selection")}><Play size={17} /> {language === "ar" ? "ابدئي الرحلة" : "Start a drive"}</button><div className="onboarding-links"><button onClick={() => setFlow("selection")}>{t.garage}</button><button onClick={() => { setDriveStarted(true); setPanel("music"); }}>{t.music}</button><button onClick={() => { setDriveStarted(true); setPanel("settings"); }}>{t.settings}</button><button onClick={() => { setDriveStarted(true); setPanel("info"); }}>About</button></div></div>}
      {flow === "selection" && <div className="onboarding-panel step-panel"><button className="back-link" onClick={() => setFlow("menu")}><ChevronLeft size={16} /> Back</button><div className="eyebrow">01 / CAR SELECTION</div><h2>{t.selectCar}</h2><div className="selection-hero"><img src={`${import.meta.env.BASE_URL}assets/veloura-car-cutout.webp`} alt="Selected car" /><strong>{getVehicle(carId).name}</strong><small>{getVehicle(carId).subtitle} · {getVehicle(carId).topSpeed} km/h</small></div><div className="car-list">{VEHICLES.map((car) => <button key={car.id} className={`car-option ${car.id === carId ? "chosen" : ""}`} onClick={() => chooseCar(car.id)}><span className="car-swatch" style={{ background: car.defaultColor }} /><div><strong>{car.name}</strong><small>{car.subtitle}</small></div><span className="car-stat">{car.topSpeed}<small>km/h</small></span></button>)}</div><button className="primary-cta" onClick={() => setFlow("customize")}>Continue <ChevronRight size={17} /></button></div>}
      {flow === "customize" && <div className="onboarding-panel step-panel"><button className="back-link" onClick={() => setFlow("selection")}><ChevronLeft size={16} /> Back</button><div className="eyebrow">02 / CUSTOMIZATION</div><h2>Make it yours</h2><div className="selection-hero"><img src={`${import.meta.env.BASE_URL}assets/veloura-car-cutout.webp`} alt="Car customization preview" /><strong>{getVehicle(carId).name}</strong><small>Selected color updates instantly</small></div><div className="color-grid">{COLORS.map((item) => <button key={item} aria-label={item} className={`color-swatch ${item === color ? "picked" : ""}`} style={{ background: item }} onClick={() => chooseColor(item)} />)}</div><button className="primary-cta" onClick={() => setFlow("environment")}>Choose environment <ChevronRight size={17} /></button></div>}
      {flow === "environment" && <div className="onboarding-panel step-panel"><button className="back-link" onClick={() => setFlow("customize")}><ChevronLeft size={16} /> Back</button><div className="eyebrow">03 / ENVIRONMENT</div><h2>Choose your route</h2><div className="environment-grid">{[["coastal", "Coastal Road", "Sunset coastline"], ["garden", "Garden Loop", "Quiet green turns"], ["town", "Night Town", "Warm city lights"]].map(([id, name, desc]) => <button key={id} className={`environment-card ${environment === id ? "chosen" : ""}`} onClick={() => setEnvironment(id)}><span className={`environment-swatch ${id}`} /><strong>{name}</strong><small>{desc}</small></button>)}</div><button className="primary-cta" onClick={() => setFlow("preDrive")}>Review drive <ChevronRight size={17} /></button></div>}
      {flow === "preDrive" && <div className="onboarding-panel step-panel"><button className="back-link" onClick={() => setFlow("environment")}><ChevronLeft size={16} /> Back</button><div className="eyebrow">READY / PRE-DRIVE</div><h2>Ready to drive?</h2><div className="drive-summary"><div><span>CAR</span><strong>{getVehicle(carId).name}</strong></div><div><span>COLOR</span><i style={{ background: color }} /></div><div><span>ROUTE</span><strong>{environment === "coastal" ? "Coastal Road" : environment === "garden" ? "Garden Loop" : "Night Town"}</strong></div><div><span>CONTROL</span><strong>{controlMode === "motion" ? t.motion : controlMode === "wheel" ? t.wheel : t.buttons}</strong></div></div><button className="primary-cta onboarding-primary" onClick={startFromPreDrive}><Play size={17} /> START DRIVE</button></div>}
    </section>}
  </div>;
}
